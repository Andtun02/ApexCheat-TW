...
public:
	...
	uint32_t getPid(const char* process_name);

	bool attach(const char* proc_name, std::string& dir);
	...
	std::string readstr(uintptr_t addr, int maxLength = 1000)
	{
		std::string str = "";
		std::vector<char> chars(maxLength);
		read_memory(addr, (uintptr_t)chars.data(), maxLength);
...
#define memRead(src_addr,type)  kernelHandler.readmem<type>(src_addr)
#define memWrite(dst_addr,var,type)  kernelHandler.writemem<type>(dst_addr,var)
