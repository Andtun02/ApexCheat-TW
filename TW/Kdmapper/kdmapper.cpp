...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool destroyHeader, mapCallback callback, NTSTATUS* exitCode) {
	...
	uint64_t kernel_image_base = AllocMdlMemory(iqvw64e_device_handle, image_size, &mdlptr);
	...
	do {
		...
		NTSTATUS status = 0;
		if (!intel_driver::CallKernelFunction(iqvw64e_device_handle, &status, address_of_entry_point, (false ? realBase : param1), param2)) {
